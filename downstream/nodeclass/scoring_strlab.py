...
for line in r:
    ...
    n=long(content[0])
    if n not in val_nod:
        val_nod[n]=len(val_nod)+1
        nod_val[len(val_nod)]=n
r.close()
...
for line in r:
    ...
    if n in val_nod:
        #x=long(content[0])#+1
        #if x not in n_2_id:
        #    n_2_id[x]=1+len(n_2_id)
        x=val_nod[n]
        if x>mx_node:
            mx_node=x
        for lab in content[1].split(','):
            if lab not in l_2_id:
                l_2_id[lab]=1+len(l_2_id)
            lab=l_2_id[lab]
            y=long(lab)
            if y>mx_label:
                mx_label=y
...
model = Word2Vec.load_word2vec_format(embd_file, binary=False) #,
                                      #norm_only=False)
AA=[]
for node in range(mx_node+1):
    if node in nod_val:
        AA.append(model[str(nod_val[node])])
features_matrix=np.array(AA)
#features_matrix = numpy.asarray([model[str(nod_val[node])] for node in range(mx_node+1)])
